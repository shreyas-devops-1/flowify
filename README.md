...
4. For production, add `https://your-domain.example/api/auth/spotify/callback` exactly (including HTTPS and path).
...
| `NEXT_PUBLIC_APP_URL` | Public app origin, used to form the exact callback URL. |
...
3. Set all environment variables above, with `NEXT_PUBLIC_APP_URL` set to the production origin.
...
- **Login fails:** ensure the redirect URI is an exact Dashboard match and all server variables are set.
